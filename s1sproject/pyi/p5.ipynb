...
def calculateProbability(x, mean, stdev):
    if stdev == 0:
        # if standard deviation is zero, return a very small probability value
        return 1e-200
    else:
...
def predict(summaries, inputVector):
    ...
    for classValue, probability in probabilities.items():  # assigns that class which has highest probability
